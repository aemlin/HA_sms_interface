...
static std::string buildStatusPayload(const char* state, int rssi,
...
void test_heartbeat_millis_overflow() {
    ...
    // last = UINT32_MAX - 30000, now = 30001  →  elapsed = 60002
...
void test_status_payload_online() {
    std::string payload = buildStatusPayload("online", -72, 3600);
    JsonDocument doc;
...
void test_status_payload_offline() {
    std::string payload = buildStatusPayload("offline", 0, 0);
    JsonDocument doc;
...
void test_status_payload_is_valid_json() {
    std::string payload = buildStatusPayload("online", -85, 120);
    JsonDocument doc;
    TEST_ASSERT_TRUE(deserializeJson(doc, payload) == DeserializationError::Ok);
}
...
void test_status_payload_uptime_large() {
    ...
    std::string payload = buildStatusPayload("online", -60, 1200000UL);
    JsonDocument doc;
    deserializeJson(doc, payload);
